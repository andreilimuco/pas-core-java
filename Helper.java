		  ...
		  for(CustomerAccount cus : customerList) {
			  if(cus.getAccNo().equals(customerAccNo)) { //checks if entered account number matches with existing account number before allowing user to get a policy quote and buy the policy
				  ...
				  for(int i = 0; i < numOfVehicle; i++) { //loops depend on number of vehicle entered
					  ...
					  System.out.print("Enter make (Toyota, Ford, BMW, etc): ");
					  String make = sc.next() + sc.nextLine();
					  System.out.print("Enter model: ");
					  String model = sc.next() + sc.nextLine();
					  int year = (int) validateNumber("Enter year (yyyy): ");
					  System.out.print("Enter type (4-door sedan, 2-door sports car, SUV, or truck): ");
					  String type = sc.next() + sc.nextLine();
					  System.out.print("Enter fuel type (Diesel, Electric, Petrol): ");
	...
	public void searchAccount() {
		System.out.println();
		  ...
		  for(CustomerAccount cusAcc : customerList) {
			  if(cusAcc.getFirstName().equalsIgnoreCase(cusFName) && cusAcc.getLastName().equalsIgnoreCase(cusLName)) { //displays all the details of a customer account
				  matchName++;
				  System.out.println("--------------------------------------------");
				  ...
				  System.out.println();
				  System.out.println("--------------------------------------------");
				  System.out.println("~ List of Policyholders and their Policies ~");
				  System.out.println("--------------------------------------------");
		  ...
		  for(CustomerAccount customerAcc : customerList) {
			  for(Policy policy : customerAcc.getPolicies()) {
				  if(policy.getPolicyNo().equals(policyNumber)) { //displays all the information associated with policy
					  ...
					  System.out.println("Birthdate: " + policy.getPolicyHolder().getBirthDate());
					  ...
					  for(Vehicle veh : policy.getVehicles()) {
						  ...
						  System.out.println("Premium Charged: " + String.format("$%.2f", veh.getPremiumCharge()));
						  System.out.println();
					  }
					  System.out.println("============================================");
		  ...
		  for(CustomerAccount cusAcc : customerList) {			  
			  for(Policy policy : cusAcc.getPolicies()) {
				  for(Claim claim : policy.getClaims()) {
					  if(claim.getClaimNo().equalsIgnoreCase(claimNo)) { //displays claim details if user entered claim number matches with existing claim number
						  claimMatch++;
						  System.out.println("--------------------------------------------");
